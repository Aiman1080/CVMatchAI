...
export function AdminSidebar() {
  ...
  const sidebarContent = (
    <>
      <div className="p-6 border-b border-gray-800 flex items-center justify-between">
        ...
        <button onClick={() => setMobileOpen(false)} className="md:hidden p-1 text-gray-400 hover:text-gray-200 rounded transition-colors">
          <X size={20} />
        </button>
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                active ? 'bg-purple-900/50 text-purple-300' : 'text-gray-400 hover:bg-gray-900 hover:text-gray-200'
              )}
            >
              <item.icon size={18} className={active ? 'text-purple-400' : 'text-gray-500'} />
              {item.label}
            </Link>
          )
        })}
      ...
      <div className="px-4 pb-2">
        ...
        >
          <span className="text-xs font-medium text-gray-400">{isDark ? 'Dark mode' : 'Light mode'}</span>
  ...
  return (
    <>
      ...
      <aside className={cn('fixed left-0 top-0 h-full w-64 bg-gray-950 border-r border-gray-800 flex flex-col transition-transform duration-300 ease-in-out', 'md:translate-x-0 md:z-40', mobileOpen ? 'translate-x-0 z-[51]' : '-translate-x-full z-[51]')}>
        {sidebarContent}
      </aside>
    </>
  )
