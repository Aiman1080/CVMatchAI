...
export function AIInsightsPanel({ candidates, totalCandidates, avgScore }: Props) {
  ...
  const insights = [
    avgScore >= 70
      ? { icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-950/30', text: `Strong candidate pool — avg. ${avgScore}% match score` }
      : { icon: AlertCircle, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-950/30', text: `Avg. match score is ${avgScore}% — consider broadening search` },
    candidates.filter(c => c.matchScore >= 75).length > 0
      ? { icon: TrendingUp, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-950/30', text: `${candidates.filter(c => c.matchScore >= 75).length} candidate(s) scored 75%+ — ready for interview` }
      : { icon: AlertCircle, color: 'text-gray-500', bg: 'bg-gray-50 dark:bg-gray-800', text: 'Upload more CVs to get AI-powered insights' },
    { icon: Sparkles, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-950/30', text: `AI analyzed ${totalCandidates} candidate${totalCandidates !== 1 ? 's' : ''} in your pipeline` },
  ]
  return (
      ...
      <CardContent className="space-y-3">
        ...
        {candidates.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
            <p className="text-xs font-semibold text-gray-700 dark:text-gray-300 mb-2">Top Matches</p>
