  ...
  const plans = [
    {
      ...
      cta: t.upgrade.currentPlan, disabled: true,
    },
    {
      ...
      cta: t.upgrade.upgradeToPro, popular: true, disabled: false,
    },
  ]
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {plans.map(plan => (
              <div key={plan.name} className={`relative rounded-2xl border-2 p-8 ${plan.color} bg-white dark:bg-gray-800`}>
                {(plan as any).popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2"><span className="gradient-bg text-white text-xs px-3 py-1 rounded-full font-semibold">{t.upgrade.mostPopular}</span></div>}
