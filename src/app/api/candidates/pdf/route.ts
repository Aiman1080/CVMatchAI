  ...
  const rows = candidates.map((c, i) => {
    ...
    <div class="candidate-card">
      <div class="candidate-header">
        ...
        <div class="candidate-avatar">${e(c.firstName?.[0] || '?')}${e(c.lastName?.[0] || '')}</div>
        <div class="candidate-info">
          <div class="candidate-name">${e(c.firstName)} ${e(c.lastName)}</div>
          <div class="candidate-meta">
            ${c.email ? `<span>✉ ${e(c.email)}</span>` : ''}
            ${c.phone ? `<span>📞 ${e(c.phone)}</span>` : ''}
            ${c.vacancy?.title ? `<span>📌 ${e(c.vacancy.title)}</span>` : ''}
          </div>
          <div style="display:flex;gap:8px;align-items:center;margin-top:4px">
            <span class="status-badge" style="background:${statusBg[c.status] || '#f9fafb'};color:${statusColor[c.status] || '#374151'}">${e(c.status)}</span>
            ${c.recommendation ? `<span class="rec-badge">${c.recommendation === 'strong_yes' ? '🎯 Strongly recommended' : c.recommendation === 'yes' ? '✅ Recommended' : c.recommendation === 'maybe' ? '🤔 To consider' : '❌ Not recommended'}</span>` : ''}
            ${c.liked ? '<span style="color:#f59e0b">⭐ Favorite</span>' : ''}
            ${c.priority ? '<span style="color:#ef4444">🚩 Priority</span>' : ''}
          </div>
        </div>
      ...
      ${c.summary ? `<div class="summary">${e(c.summary)}</div>` : ''}
      <div class="details-grid">
        ${strengths.length ? `<div class="detail-section strengths"><div class="detail-title">✅ Strengths</div>${strengths.map((s: string) => `<div class="detail-item">• ${e(s)}</div>`).join('')}</div>` : ''}
        ${weaknesses.length ? `<div class="detail-section weaknesses"><div class="detail-title">⚠️ Areas of attention</div>${weaknesses.map((w: string) => `<div class="detail-item">• ${e(w)}</div>`).join('')}</div>` : ''}
      </div>
      ${skills ? `<div class="skills-row"><span class="skills-label">Skills:</span> <span class="skills-list">${e(skills)}</span></div>` : ''}
      ${c.experience ? `<div class="detail-block"><div class="detail-title">💼 Experience</div><div class="detail-text">${e(c.experience)}</div></div>` : ''}
      ${c.education ? `<div class="detail-block"><div class="detail-title">🎓 Education</div><div class="detail-text">${e(c.education)}</div></div>` : ''}
    </div>`
  }).join('')
  ...
  <div class="page">
    <div class="header">
      ...
      <div class="header-meta">
        <div class="report-title">Candidate Report</div>
        <div class="vacancy-name">${e(vacancyTitle)}</div>
        <div>${date}</div>
        <div>Generated by ${e(generatedBy)}${company ? ` · ${e(company)}` : ''}</div>
      </div>
    </div>
